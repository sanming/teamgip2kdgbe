 ...
 */
@Entity
@Table(name = "RepairDetails")
@Inheritance(strategy = InheritanceType.SINGLE_TABLE)
@NamedQueries(
        {
                @NamedQuery(name = "Repairdetails.findAll", query = "SELECT r FROM RepairDetails r"),
                @NamedQuery(name = "Repairdetails.findByRepairdetailsid", query = "SELECT r FROM RepairDetails r WHERE r.repairDetailsId = :repairdetailsid"),
                @NamedQuery(name = "Repairdetails.findByDefect", query = "SELECT r FROM RepairDetails r WHERE r.defect = :defect"),
                @NamedQuery(name = "Repairdetails.findByDescription", query = "SELECT r FROM RepairDetails r WHERE r.description = :description"),
                @NamedQuery(name = "Repairdetails.findByEvaluation", query = "SELECT r FROM RepairDetails r WHERE r.evaluation = :evaluation"),
                @NamedQuery(name = "Repairdetails.findByRebuttal", query = "SELECT r FROM RepairDetails r WHERE r.rebuttal = :rebuttal"),
                @NamedQuery(name = "Repairdetails.findByRating", query = "SELECT r FROM RepairDetails r WHERE r.rating = :rating"),
                @NamedQuery(name = "Repairdetails.findByStatus", query = "SELECT r FROM RepairDetails r WHERE r.status = :status"),
                @NamedQuery(name = "Repairdetails.findByPricemodel", query = "SELECT r FROM RepairDetails r WHERE r.priceModel = :pricemodel"),
                @NamedQuery(name = "Repairdetails.findByDuedate", query = "SELECT r FROM RepairDetails r WHERE r.dueDate = :duedate"),
                @NamedQuery(name = "Repairdetails.findBySubmitdate", query = "SELECT r FROM RepairDetails r WHERE r.submitDate = :submitdate"),
                @NamedQuery(name = "Repairdetails.findByAssigned", query = "SELECT r FROM RepairDetails r WHERE r.assigned = :assigned")
        })
public class RepairDetails implements Serializable
{
    @Column(name = "RepairDetailsId", nullable = false)
    @Id
    @GeneratedValue
    private Integer repairDetailsId;
    @Column(name = "Defect", nullable = true, length = 255)
    private String defect;
    @Column(name = "Description", nullable = true, length = 255)
    private String description;
    @Column(name = "Evaluation", nullable = true, length = 255)
    private String evaluation;
    @Column(name = "Rebuttal", nullable = true, length = 255)
    ...
    @Column(name = "PriceModel", nullable = true, length = 10)
    @Enumerated(EnumType.STRING)
    ...
    /**
     * Don't use this method directly. Change the status of repairs through the
     * RepairService.
     *
